...
void connection_manager(int remote_sock, Field& field, int thread_id)
{
	char command_buffer;
	User temp;
	int ret;
	...
	while (1)
	{
		...
		// rec id
		ret = recv(remote_sock, &temp.uid, sizeof(temp.uid), 0);
		printf("ret=%d\tbuff_id=%d\tthread_id=%d\n", ret, temp.uid, thread_id);
		//rec command
		ret = recv(remote_sock, &command_buffer, sizeof(command_buffer), 0);
...
int main()
{
	...
	while (1)
	{
		...
		threads.push_back(std::thread(connection_manager, remote_sock, std::ref(field), thread_id++));
		remote_sock = 0;
		memset(&listen_sock_addr, 0, sizeof(remote_sock_addr));
		if(can_close)
		{
